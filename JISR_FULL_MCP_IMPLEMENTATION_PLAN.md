...
- ~~Why does `GET /employees` return `line_manager.id` as null?~~ **Resolved 2026-08-31:** it does not. The list returns `{ guid, full_name }` where basic_info returns `{ id, name }`. Same UUID, two dialects. Handled.
- Why do the two employee endpoints return different shapes for the same entity (`line_manager`, `location`, `avatar`), and is either canonical?
- Why does the OpenAPI document mark `status`, `leave_type`, `accrual_type`, `paygroup_id` and `pay_period` as optional when the API rejects requests that omit them?
...
| 2026-08-29 | Audit records go to stderr as structured JSON; nothing written to disk | A live-only release with no database has nowhere durable to put them, and writing financial-access records to disk recreates the retention problem the design avoided | Retention and forwarding become the operator's documented responsibility (FR-038a) |
| 2026-08-29 | The manager profile reaches direct reports only | Jisr exposes a single-level `line_manager` field; an indirect tree would have to be derived client-side and then trusted for an authorization decision | No reporting-tree traversal; the authorization matrix stays bounded (FR-019a) |
| 2026-08-29 | Adapter parity is a requirement, not a plan intention | The five-client promise rests on both protocol adapters behaving identically, which nothing was holding us to | FR-002a and SC-014 added; parity test moved into the foundational phase beside the adapters |
| 2026-08-29 | Constitution Principle IV's storage clauses recorded as dormant rather than satisfied | Nothing is stored this release, so they are vacuously true — and a future contributor adding a cache would reasonably read that as compliance | Recorded in spec Assumptions and the plan's Constitution Check; constitution left at v1.0.0 |
| 2026-08-29 | No server-side latency target for this release | Upstream rate limits and response times are undocumented, so any figure would be invented rather than derived | Page size and per-invocation record ceilings remain the enforced bounds; a latency target waits for real tenant data |
| 2026-08-29 | Releases are tag-triggered, automated, and published with provenance from GitHub | A package carrying HR data access should never be published from a maintainer's laptop, and adopters need to know which protocol and Jisr snapshot each release was verified against | FR-044a and FR-044b added; branch protection, release workflow, and a release-consistency check become pre-publication tasks |
| 2026-08-29 | Do not implement a Jisr key capability probe | Jisr documents no permission-discovery endpoint, and probing by trial call would guess at semantics and generate spurious upstream denials | Capability reporting says `unknown` rather than `false` for key permission. Resolve with Jisr before claiming Definition of Done item 4 |
| 2026-08-29 | Generate the authorization matrix and coverage docs from code | A hand-maintained authorization matrix is wrong the first time a profile changes, and a wrong security document is worse than none | `npm run docs:generate`; both files carry a generated-header |
| 2026-08-29 | Return prompt-injection content verbatim rather than sanitizing it | Rewriting record content would corrupt legitimate names, and Arabic text especially | The guarantee is that content cannot ACT: authorization never reads record content. Asserted across five injection payloads |
| 2026-08-29 | Require JISR_SUBJECT_EMPLOYEE_ID for the employee_self and manager profiles | Both are defined relative to a person, and without that identity they resolved to an empty reachable set. The server started cleanly and returned nothing forever, which is indistinguishable from "this person manages nobody" | FR-019b added; startup refuses in both directions -- missing where required, and present where the profile is not person-scoped |
| 2026-08-30 | Parse upstream records with a LOOSE schema, not a strict one | A strict z.object() strips unknown keys at the schema boundary, so drift detection never saw them. Live Jisr returns a `bank` field the schema did not declare and nothing recorded it | Employee and webhook schemas use z.looseObject; the mapper allowlist remains the only thing keeping fields out, and drift is now actually detected (FR-027) |
| 2026-08-30 | Map the employee UUID from `id` as well as `employee_id` | Live Jisr returns it as `id`; the documentation calls it `employee_id`. Mapping only the documented name left employeeId null on every record | Both accepted. Reachability matching by UUID now works |
| 2026-08-30 | `line_manager.id` confirmed as the employee UUID; manager profile still blocked | Resolved live: it is a UUID string matching the employee `id`. But `GET /employees` returns it null on every record, so manager scoping on the list has nothing to match | Manager profile fails closed and returns an empty list. Safe but unusable; do not enable in production until Jisr confirms whether the list can populate it |
| 2026-08-31 | Accept both `line_manager` dialects and unblock the manager profile | The 2026-08-30 null finding was our own schema reading `id` where the list sends `guid`. Verified by fetching the same employee through both endpoints | Manager profile verified working live: 5 of 100 records for a manager with 4 direct reports, scope warning raised, no salary in the result. Also narrowed R2a: basic_info honours the finance permission; only the list ignores it |
| 2026-08-31 | Employee codes accept strings as well as integers | Jisr's documentation says "Array of Integers" with example [1,2,3]; the live AZMX tenant uses alphanumeric codes such as AZMX117. Found by answering a real question through the protocol | Leave summary input accepts both; the docs example is a small-tenant artifact, not the contract |
| 2026-08-30 | Make `status`, `leave_type`, `accrual_type`, `paygroup_id` and `pay_period` required | Jisr rejects requests omitting them with a 400, despite its OpenAPI document marking every one optional. Its own prose descriptions mark them required | Tool inputs now require them, so a caller is told at the schema rather than by an upstream 400 |
| 2026-08-30 | Classify `bank`, `terminate_date` and `delete_date`, observed live but undocumented | `bank` holds banking details and was arriving unclassified | `bank` is financial_confidential; the other two are internal_operational |
| 2026-08-29 | Repository relocated out of Google Drive to `~/Projects/jisr-mcp` | Drive's file provider permits creation but rejects deletion, so `npm install` fails with EPERM; the same install succeeds in under a second on a local path | Full git history cloned; the Drive copy is stale and superseded. GitHub becomes the backup once the remote is added |
| 2026-08-29 | TypeScript pinned to 6.x rather than the latest 7.x | `typescript-eslint@8.68` declares `typescript >=4.8.4 <6.1.0`; TypeScript 7 would disable type-aware linting entirely | TypeScript 6.0.3 with ESLint 10 and Vitest 4. TypeScript is a devDependency only, so consumers of the published package are unaffected |
