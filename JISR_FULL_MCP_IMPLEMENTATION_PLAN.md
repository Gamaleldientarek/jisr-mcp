...
- **Why does `GET /employees` return `line_manager.id` as null when `GET /employees/basic_info` populates it?** Is there a parameter to include it? Without it, manager-scoped access to the employee list is impossible without an N+1 fetch.
