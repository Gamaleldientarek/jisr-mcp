...
| `GET /employees` returns `line_manager.id` as null | The `manager` role profile (FR-018a, FR-019a) | **Resolved and superseded.** `line_manager.id` is confirmed as the employee UUID (live, 2026-08-30), but the employee LIST never populates it -- only `basic_info` does. Manager scoping on the list therefore has nothing to match and fails closed to an empty result. Ask Jisr whether the list can include it; otherwise the manager profile needs an N+1 fetch or must be withdrawn |
