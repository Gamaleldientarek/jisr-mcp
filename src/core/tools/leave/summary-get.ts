...
export const leaveSummaryTool: ToolDefinition<{
  employeeCodes: (string | number)[];
  leaveType: string;
  pageSize?: number;
}> = {
  name: 'jisr_employee_leave_summary_get',
  title: 'Get annual leave summary',
  description: `Retrieves leave balances for the given employees: opening balance, used, pending, reserved, year-end balance and unpaid deduction. Takes employee CODES (e.g. "EMP0117"), not UUIDs. Jisr accepts at most ${LEAVE_SUMMARY_MAX_CODES} codes per upstream call; larger requests are split automatically up to ${MAX_RECORDS_PER_INVOCATION}.`,
  inputShape: {
    // Documentation says integers; real tenants use alphanumeric codes such as
    // "EMP0117". Both are accepted.
    employeeCodes: z
      .array(z.union([z.string(), z.number()]))
      .min(1)
      .max(MAX_RECORDS_PER_INVOCATION)
      .describe('Employee codes, as returned by jisr_employees_list (e.g. "EMP0117").'),
