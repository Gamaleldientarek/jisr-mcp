...
| ~~`line_manager` semantics~~ **Resolved 2026-08-31** | The `manager` role profile | `line_manager` is the employee UUID, delivered as `{ id, name }` by basic_info and `{ guid, full_name }` by the list. Both dialects normalized; manager scoping verified live (5 of 100 records for a manager with 4 reports). The earlier null finding was this project's own schema reading the wrong key |
